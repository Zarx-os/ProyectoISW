    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (usuario == 0) {
            ...
            var params: ConstraintLayout.LayoutParams =
                btnFrag.layoutParams as ConstraintLayout.LayoutParams
            supportFragmentManager.beginTransaction().replace(R.id.contenedor, fragmentInicio)
                .commit()
            btnFrag.setOnClickListener {
                ...
                when (cont) {
                    0 -> {
                        ...
                        params.width = 160
                        params.height = 80
                        btnFrag.animate().x(520F).y(1250F).setDuration(1500).start()
                        cont++
                    }
                    1 -> {
                        if (prefs.getVal() == 1) {
                            ...
                            Toast.makeText(
                                this,
                                "Recuerda que debes terminar el formulario",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
                    2 -> {
                        if (prefs.getVal() == 1) {
                            if (fragmentAnsiedad.getRes() > fragmentEstres.getRes()) {
                                prefs.saveTrans(1)
                            } else if (fragmentEstres.getRes() > fragmentAnsiedad.getRes()) {
                                prefs.saveTrans(2)

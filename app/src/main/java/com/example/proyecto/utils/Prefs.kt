...
class Prefs(val context: Context) {
    ...
    val SHARED_MD = "modulos"
    val SHARED_MDUSUARIO = "modulosuser"
    ...
    fun saveM(number: Int) {
        storage.edit().putInt(SHARED_MD, number).commit()
    }

    fun saveMUser(number: Int) {
        storage.edit().putInt(SHARED_MDUSUARIO, number).commit()
    }

    fun getMUser(): Int {
        return storage.getInt(SHARED_MDUSUARIO, 0)!!
    }

    fun getM(): Int {
        return storage.getInt(SHARED_MD, 0)!!
    }
